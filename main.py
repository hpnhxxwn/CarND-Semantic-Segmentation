...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    upsampled_vgg_layer7_2x = tf.layers.conv2d_transpose(layer7_conv1x1, num_classes, 4, 2, padding='same',
    ...
    upsampled_first_skip_layer = tf.layers.conv2d_transpose(first_skip_layer, num_classes, 4, 2, padding='same',
                                                                 ...
                                                                 kernel_regularizer=tf.contrib.layers.l2_regularizer(
                                                                     L2_REG))
    second_skip_layer = tf.add(upsampled_first_skip_layer, layer3_out_scaled_conv1x1)

    output = tf.layers.conv2d_transpose(
        inputs=second_skip_layer,
        filters=num_classes,
        kernel_size=16,
        strides=(8, 8),
        padding="same",
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for e in range(epochs):
        for image, targets in get_batches_fn(batch_size):
            ...
            print("Epoch: {}".format(e + 1), "/ {}".format(epochs), " Loss: {:.3f}".format(loss), " Time: ",
                  str(timedelta(seconds=(time.time() - s_time))))
            # if (e + 1) % 30 == 0:  # Save every 20 epochs
            #     save_path = saver.save(sess, os.path.join(out_dir, 'cont_epoch_' + str(e) + '.ckpt'))
